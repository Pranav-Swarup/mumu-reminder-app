        ...
        if (displayList.isNotEmpty()) {
            items(displayList, key = { it.id }) { task ->
                ...
                val repeatLabel = when (task.repeatType) {
                    ...
                    RepeatType.MONTHLY -> "Monthly on the ${task.monthDay}${ordSuffix(task.monthDay)}"
                    RepeatType.NONE -> task.dueTime?.let { timeFormat.format(Date(it)) } ?: ""
                }
                val desc = if (task.dueTime != null) {
                    "${timeFormat.format(Date(task.dueTime))} - $repeatLabel"
                } else repeatLabel

                SwipeableTaskCard(
                    task = task.copy(description = desc),
                    onComplete = { onSwipeComplete(task) },
                    onDelete = { onSwipeDelete(task) },
...
private fun ordSuffix(day: Int): String = when {
    day in 11..13 -> "th"
    day % 10 == 1 -> "st"
    day % 10 == 2 -> "nd"
    day % 10 == 3 -> "rd"
